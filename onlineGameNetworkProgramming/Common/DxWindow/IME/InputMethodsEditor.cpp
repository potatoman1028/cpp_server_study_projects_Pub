...
{
	imc = ::ImmGetContext( ::AfxGetMainWnd()->m_hWnd );
	if( imc == 0 )
		return false;
...
{
	::ImmReleaseContext( ::AfxGetMainWnd()->m_hWnd, imc );	
}
...
{
/*
    if (ImmIsIME(keyLayout) && m_property & IME_PROP_AT_CARET)
		ClearData();
	...
	if(Enter()) {
		for (int i = 0; i < MAX_LISTCAND; i++) {
			...
			if (m_property & IME_PROP_AT_CARET) {
				...
				if (!ImmGetCandidateWindow(imc, i, &cf))
					continue;
	...
	return DefWindowProc(handleWnd,WM_INPUTLANGCHANGE, command, data);
...
long InputMethodsEditor::onIMESetContext( WPARAM command, LPARAM data )
{
	...
	return ::DefWindowProc( ::AfxGetMainWnd()->m_hWnd, WM_IME_SETCONTEXT, command, data );
}
