																								...
																								SBStyle style, int x, int y, Window* parent )
{
	...
	//  Create widows
	{
		...
		if( decButton->create( decBtnFileName, 0, 0, Attr_ShowWindow | Attr_Movable, this ) == false )
			return false;

		incButton = new ButtonCtrl( CID_SCROLLBAR_INCBUTTON, display );
		if( incButton->create( incBtnFileName, 0, 0, Attr_ShowWindow | Attr_Movable, this ) == false )
			return false;
	}
