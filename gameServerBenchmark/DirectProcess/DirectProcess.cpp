// DirectProcess.cpp : Defines the entry point for the console application.
//
...
#include "../Network/NetIocp2.h"
#include "../Network/SimpleServer2.h"
...
class CTestServer : public CSimpleServer2<CTestNet>
{
public:
	VOID Begin(VOID)
	{
...
protected:
	...
	{
		m_oProfile.Begin(L"DirectProc OnConnected");
		Overhead();
		m_oProfile.End(L"DirectProc OnConnected");
		Dump();
	}

	VOID OnDisconnected(CTestNet *poNetObj)
	{
		m_oProfile.Begin(L"DirectProc OnDisconnected");
		Overhead();
		m_oProfile.End(L"DirectProc OnDisconnected");
		Dump();
	}

	VOID OnRead(CTestNet *poNetObj, BYTE *pReadBuf, DWORD dwLen)
	{
		m_oProfile.Begin(L"DirectProc OnRead");
		Overhead();
		m_oProfile.End(L"DirectProc OnRead");
	...
	{
		m_oProfile.Begin(L"DirectProc OnWrite");
		Overhead();
		m_oProfile.End(L"DirectProc OnWrite");
		Dump();
	}
};
