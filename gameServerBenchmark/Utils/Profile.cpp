...
VOID CProfile::Begin(LPWSTR lpwName)
{
	...
	m_oSamples[i].dAccumulator = 0.0;
	m_oSamples[i].dStartTime = getExactTime();
	m_oSamples[i].dChildrenSampleTime = 0.0;
}
...
VOID CProfile::DumpProfileOutput(VOID)
{
	...
	CLog::WriteLog(L"   Ave  :   Min   :   Max   :       # : Profile Name\n");
	//wprintf(L"---------------------------------------------------------------------------\n");
	CLog::WriteLog(L"---------------------------------------------------------------------------\n");
	...
	while (i < MAX_PROFILE_SAMPLES && m_oSamples[i].bValid == TRUE)
	{
		...
		PercentTime = (SampleTime / (m_dEndProfile - m_dBeginProfile)) * 100.0;
