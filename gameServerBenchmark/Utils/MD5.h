...
static const char b64t[64+1] =
"./0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
static char* __stpncpy(char *dest,const char* a,int n)
{
	strncpy(dest,a,n);
...
libcs.  */
static char*	__md5_crypt_r (const char* key,const char* salt,char* buffer,int buflen)
{
...
static char*	md5crypt (const char *key, const char *salt,char* buffer,int buflen)
{
	return __md5_crypt_r (key, salt, buffer, buflen);
}

void md5hash__(const char* source,int len,char* target)
{
	struct md5_ctx ctx;

	md5_init_ctx (&ctx);
	md5_process_bytes (source, len, &ctx);
	md5_finish_ctx (&ctx, target);
}