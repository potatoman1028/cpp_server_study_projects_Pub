...
bool PlayerManager::AddPlayer( Player* pPlayer )
{
	...
	if( player_it != m_mapPlayer.end() )
	{
		LOG( LOG_ERROR_NORMAL , 
			"SYSTEM | PlayerManager::AddPlayer() | PKey(%d)는 이미 m_mapPlayer에 있습니다.",
			pPlayer->GetPKey() );
		return false;
	}
	//플레이어 고유키할당
...
bool PlayerManager::RemovePlayer( Player* pPlayer )
{
	...
	{
		LOG( LOG_ERROR_NORMAL , 
			"SYSTEM | PlayerManager::RemovePlayer() | PKey(%d)는 m_mapPlayer에 없습니다.",
			pPlayer->GetPKey() );
		return false;
	}
...
Player* PlayerManager::FindPlayer( DWORD dwPkey )
{
	...
	{
		LOG( LOG_ERROR_NORMAL , 
			"SYSTEM | PlayerManager::FindPlayer() | PKey(%d)는 m_mapPlayer에 없습니다.",
			dwPkey );
		return NULL;
	}
...
void PlayerManager::CreateTempPlayer( int nTempPlayerCnt )
{
	...
	LOG( LOG_INFO_LOW , 
		"SYSTEM | PlayerManager::DestroyTempPlayer() | 가상 플레이어 생성 성공!!");
}
...
void PlayerManager::DestroyTempPlayer()
{
	...
	delete [] m_pTempPlayer;
	LOG( LOG_INFO_LOW , 
		"SYSTEM | PlayerManager::DestroyTempPlayer() | 가상 플레이어 파괴 성공!!");
}
