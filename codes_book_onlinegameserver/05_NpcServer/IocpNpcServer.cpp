...
bool IocpNpcServer::OnAccept( Connection *lpConnection )
{
	...
	LOG( LOG_INFO_LOW , 
		"SYSTEM | IocpNpcServer::OnAccept() | IP[%s] Socket[%d] 접속 Connection Cnt[%d]",
...
bool IocpNpcServer::OnRecv(Connection* lpConnection,  DWORD dwSize , char* pRecvedMsg)
{
	...
	{
		LOG( LOG_ERROR_NORMAL , "SYSTEM | IocpNpcServer::OnRecv() | 정의되지 않은 패킷(%d)",
			usType );
		return true;
	}
...
void IocpNpcServer::OnClose(Connection* lpConnection)
{
	...
	LOG( LOG_INFO_LOW , 
		"SYSTEM | IocpNpcServer::OnClose() | IP[%s] Socket[%d] 종료 Connection Cnt[%d]",
		lpConnection->GetConnectionIp(), lpConnection->GetSocket() ,
		ConnectionManager()->GetConnectionCnt());
}
...
bool IocpNpcServer::ServerStart()
{
	...
	LOG( LOG_INFO_NORMAL , "SYSTEM | IocpNpcServer::ServerStart() | [Base_Queue] Process Pakcet 개수 : %d", initConfig.nProcessPacketCnt );
	LOG( LOG_INFO_NORMAL , "SYSTEM | IocpNpcServer::ServerStart() | [Base_Buffer] Send Buffer 개수 : %d ", initConfig.nSendBufCnt );
	LOG( LOG_INFO_NORMAL , "SYSTEM | IocpNpcServer::ServerStart() | [Base_Buffer] Recv Buffer 개수 : %d ", initConfig.nRecvBufCnt );
	LOG( LOG_INFO_NORMAL , "SYSTEM | IocpNpcServer::ServerStart() | [Base_Buffer] Send Buffer 크기 : %d ", initConfig.nSendBufSize );
	LOG( LOG_INFO_NORMAL , "SYSTEM | IocpNpcServer::ServerStart() | [Base_Buffer] Recv Buffer 크기 : %d ", initConfig.nRecvBufSize );
	LOG( LOG_INFO_NORMAL , "SYSTEM | IocpNpcServer::ServerStart() | [Base_Buffer] Send 총 할당 버퍼: %d ", initConfig.nSendBufSize * initConfig.nSendBufCnt );
	LOG( LOG_INFO_NORMAL , "SYSTEM | IocpNpcServer::ServerStart() | [Base_Buffer] Recv 총 할당 버퍼: %d ", initConfig.nRecvBufSize * initConfig.nRecvBufCnt );
	LOG( LOG_INFO_NORMAL , "SYSTEM | IocpNpcServer::ServerStart() | [Base_Connection] 접속할수 있는 최대 수 : %d", nMaxConnectionCnt );
	...
	LOG( LOG_INFO_NORMAL , "SYSTEM | IocpNpcServer::ServerStart() | 서버 시작 시간 :  %d년%d월%d일%d시%d분%d초",
